...
if ($avg_author > 0 || $avg_users > 0) {
	...
	$output = $marks = $users = '';
	if ($use_tabs) {
		$author_tab = '<li class="sc_tabs_title"><a href="#author_marks" class="theme_button">'.__('Author', 'education').'</a></li>';
		$users_tab = '<li class="sc_tabs_title"><a href="#users_marks" class="theme_button">'.__('Users', 'education').'</a></li>';
	...
	if (!$reviews_first_author || !$reviews_second_hide) {
		...
		$field["descr"] = sprintf(__("Summary rating from <b>%s</b> user's marks.", 'education'), $users) . ' ' 
			.(!isset($_COOKIE['themerex_votes']) || themerex_strpos($_COOKIE['themerex_votes'], ','.($post_data['post_id']).',')===false
				? __('You can set own marks for this article - just click on stars above and press "Accept".', 'education')
				: __('Thanks for your vote!', 'education'));
		...
		$reviews_markup .= '
			<script type="text/javascript">
				jQuery(document).ready(function() {
					THEMEREX_GLOBALS["reviews_allow_user_marks"] = '.($allow_user_marks ? 'true' : 'false').';
					THEMEREX_GLOBALS["reviews_max_level"] = '.($max_level).';
					THEMEREX_GLOBALS["reviews_levels"] = "'.trim(themerex_get_theme_option('reviews_criterias_levels')).'";
					THEMEREX_GLOBALS["reviews_vote"] = "'.(isset($_COOKIE['themerex_votes']) ? $_COOKIE['themerex_votes'] : '').'";
					THEMEREX_GLOBALS["reviews_marks"] = "'.($marks).'".split(",");
					THEMEREX_GLOBALS["reviews_users"] = '.max(0, $users).';
					THEMEREX_GLOBALS["post_id"] = '.($post_data['post_id']).';
				});
			</script>
		';
	}
