...
if (themerex_get_custom_option("show_post_author") == 'yes') {
	...
	if ($post_data['post_type']=='post') {
		$post_author_title = __('About', 'education');
		...
		if ($show_post_author_socials && function_exists('themerex_show_user_socials')) $post_author_socials = themerex_show_user_socials(array('author_id'=>$post_data['post_author_id'], 'style'=>'bg', 'size'=>'tiny', 'echo' => false));
	} else if ($post_data['post_type']=='courses') {
		$post_author_id = themerex_get_custom_option('teacher');
		if ($post_author_id) {
			global $post;
			$post = get_post($post_author_id);
			setup_postdata($post);
			$post_author_title = __('About Teacher', 'education');
			...
			if (empty($post_author_avatar)) {
				...
				$post_author_avatar = themerex_get_resized_image_tag($post_author_avatar, 75, 75);
			}
