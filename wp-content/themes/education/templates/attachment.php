...
if ( !function_exists( 'themerex_template_attachment_theme_setup' ) ) {
	...
	function themerex_template_attachment_theme_setup() {
		themerex_add_template(array(
			'layout' => 'attachment',
			'mode'   => 'internal',
			'title'  => __('Attachment page layout', 'education'),
			'thumb_title'  => __('Fullsize image', 'education'),
			'w'		 => 1150,
			'h'		 => null,
			'h_crop' => 455
		));
		...
		<article <?php post_class('post_item post_item_attachment template_attachment'); ?>>
				...
				<div class="post_thumb post_nav" data-image="<?php echo esc_url($post_data['post_attachment']); ?>" data-title="<?php echo esc_attr($post_data['post_title']); ?>">
					...
					if ( isset( $attachments[ $k-1 ] ) ) {
						...
						<a class="post_nav_item post_nav_prev" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php _e('Previous item', 'education'); ?></span>
					...
					if ( isset( $attachments[ $k+1 ] ) ) {
						...
						<a class="post_nav_item post_nav_next" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php _e('Next item', 'education'); ?></span>
								<span class="post_nav_info_description"><?php echo ($desc); ?></span>
							</span>
						</a>
						<?php
			...
			<div class="post_content">
				...
				if ($post_data['post_protected']) { 
					...
					echo ($post_data['post_content'] ? $post_data['post_content'] : __('No image description ...', 'education')); 
					wp_link_pages( array( 
						'before' => '<div class="nav_pages_parts"><span class="pages">' . __( 'Pages:', 'education' ) . '</span>', 
						'after' => '</div>',
