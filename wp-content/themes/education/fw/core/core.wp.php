...
if (!function_exists('themerex_wp_theme_setup')) {
	...
	function themerex_wp_theme_setup() {
		...
		// AJAX: Incremental custom search
//		add_action('wp_ajax_ajax_custom_search',			'themerex_callback_ajax_custom_search');
//		add_action('wp_ajax_nopriv_ajax_custom_search',		'themerex_callback_ajax_custom_search');
        add_filter('pre_get_posts',                         'themerex_custom_search_filter', 1, 20);
...
//			$title = sprintf( esc_html__( '%s: %s', 'education' ), $tax->labels->name, single_term_title( '', false ) ); 
...
if (!function_exists('themerex_show_breadcrumbs')) {
	function themerex_show_breadcrumbs($args=array()) {
		...
		$args = array_merge( array(
			'home' => esc_html__('Home', 'education'),		// Home page title (if empty - not showed)
							...
							if ($level > $args['max_levels'])
								...
								$rez_all .= (!empty($rez_all) ? $args['delimiter'] : '') 
										. '<a class="breadcrumbs_item all" href="' . get_permalink($tpl_id) . '">' 
											. sprintf(esc_html__('All %s', 'education'), themerex_get_post_format_name($post_format, false)) 
										. '</a>';
						}
					}
...
if (!function_exists('themerex_show_pagination')) {
	function themerex_show_pagination($args=array()) {
		$args = array_merge(array(
			...
			'show_pages' => 5,
			'pages_in_group' => 10,
			'pages_text' => '', 		//esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'education'),
			'current_text' => "%PAGE_NUMBER%",
			'page_text' => "%PAGE_NUMBER%",
			'first_text'=> esc_html__('&laquo; First', 'education'),
			'last_text' => esc_html__("Last &raquo;", 'education'),
			'prev_text' => esc_html__("&laquo; Prev", 'education'),
			'next_text' => esc_html__("Next &raquo;", 'education'),
			'dot_text' => "&hellip;",
			'before' => '',
			'after' => '',
			),  is_array($args) ? $args 
...
if (!function_exists('themerex_show_pagination_single')) {
	function themerex_show_pagination_single( $args ) {
		...
		if ( is_single() ) {
			?>
			<h3 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'education' ); ?></h3>
			<?php
			...
			if ($max_page > 1) {
				...
				if ($opt['style'] == 'pages') {
					...
					if (empty($opt['pages_text'])) 
						$opt['pages_text'] = __('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'education');
...
if ( !function_exists( 'themerex_callback_view_more_posts' ) ) {
	function themerex_callback_view_more_posts() {
		...
		if ($page > 0 && is_array($args) && is_array($vars)) {
			...
			$response['error'] = __('Wrong query arguments', 'education');
		}
...
if ( !function_exists( 'themerex_callback_ajax_search' ) ) {
	function themerex_callback_ajax_search() {
		...
		if (!empty($s)) {
			...
			if (empty($output)) {
				$output .= '<article class="post_item">' . __('Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'education') . '</article>';
			} else {
				$output .= '<article class="post_item"><a href="#" class="post_more search_more">' . __('More results &hellip;', 'education') . '</a></article>';
			}
			$response['data'] = $output;
		} else {
			$response['error'] = __('The query string is empty!', 'education');
		}
...
// Incremental search
if ( !function_exists( 'themerex_callback_ajax_custom_search' ) ) {
    function themerex_callback_ajax_custom_search() {
        global $_REQUEST, $post, $wp_query;
        ...
        if (!empty($data)) {
            ...
            if (!empty($data['s'])) {
                $args['s'] = esc_html($data['s']);
            }
            if (!empty($data['post_type'])) {
                $args['post_type'] = esc_html($data['post_type']);
            }
            if (!empty($data['tag']) && ($data['tag'] !=='all')) {
                $args['tag'] = esc_html($data['tag']);
            }
            if (!empty($data['category_name']) && ($data['category_name'] !=='all')) {
                $args['category_name'] = esc_html($data['category_name']);
            }
...
// Incremental search
if ( !function_exists( 'themerex_custom_search_filter' ) ) {
    function themerex_custom_search_filter($query) {
        if (is_admin() || !$query->is_main_query()) return;
        if( $query->is_search && isset($_REQUEST['custom_search']) && $query->get( 's' )){
            $post_type = !empty($_REQUEST['post_type']) ? esc_html($_REQUEST['post_type']) : false;
            if (!empty($_REQUEST['s'])) {
                $query->set('s', esc_html($_REQUEST['s']));
            }
            if ($post_type) {
...
if (!function_exists('themerex_get_calendar')) {
	function themerex_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		$calendar_caption = _x('%1$s %2$s', 'calendar caption', 'education');
		...
		if ( $prev_year+$prev_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($prev_year) . '" data-month="' . esc_attr($prev_month) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'education'), $wp_locale->get_month($prev_month), date('Y', mktime(0, 0, 0, $prev_month, 1, $prev_year)))) . '">'
		...
		if (count($posts) > (!empty($posts['done']) ? 1 : 0)) {
			...
			$link = '';
		$calendar_output .= '<th class="month_cur" colspan="5">' . ($link ? '<a href="' . esc_url($link) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'education'), $wp_locale->get_month($thismonth), date('Y', mktime(0, 0, 0, $thismonth, 1, $thisyear)))) . '">' : '') . sprintf($calendar_caption, $wp_locale->get_month($thismonth), '<span>'.date('Y', $unixmonth).'</span>') . ($link ? '</a>' : '') . '</th>';
		...
		if ( $next_year+$next_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($next_year) . '" data-month="' . esc_attr($next_month) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'education'), $wp_locale->get_month($next_month), date('Y', mktime(0, 0 , 0, $next_month, 1, $next_year))) ) . '">' 
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( !empty($posts[$day]) )
				$calendar_output .= '<a class="day_wrap" href="' . esc_url(!empty($posts[$day]['link']) ? $posts[$day]['link'] : get_day_link($thisyear, $thismonth, $day)) . '" title="' . esc_attr( is_int($posts[$day]['titles']) ? $posts[$day]['titles'].' '.__('items', 'education') : $posts[$day]['titles'] ) . "\">$day</a>";
			else
...
if (!function_exists('themerex_output_single_comment')) {
	function themerex_output_single_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php _e( 'Trackback:', 'education' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'education' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php _e( 'Pingback:', 'education' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'education' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					...
					<div class="comment_content">
						<div class="comment_info">
							...
							<span class="comment_date"><span class="comment_date_label"><?php _e('Posted', 'education'); ?></span> <span class="comment_date_value"><?php echo get_comment_date(get_option('date_format')); ?></span></span>
							<span class="comment_time"><?php echo get_comment_date(get_option('time_format')); ?></span>
						</div>
						<div class="comment_text_wrap">
							<?php if ( $comment->comment_approved == 0 ) { ?>
							<div class="comment_not_approved"><?php _e( 'Your comment is awaiting moderation.', 'education' ); ?></div>
							<?php } ?>
...
//		$post_date = themerex_get_date_or_difference($post_date_sql);
		...
		if ($post_format == 'gallery' && function_exists('themerex_sc_slider')) {
...
if ( !function_exists( 'themerex_query_add_posts_per_page' ) ) {
	...
	function themerex_query_add_posts_per_page($query) {
		...
		if (is_admin() || !$query->is_main_query() || isset($_REQUEST['custom_search'])) return;
		global $THEMEREX_GLOBALS;
		...
		} else if (function_exists('themerex_is_tribe_events_page') && themerex_is_tribe_events_page()) {
			$orderby_set = false;
			$page_id = themerex_get_template_page_id('tribe-events/default-template');
		} else if (function_exists('themerex_is_learndash_page') && themerex_is_learndash_page()) {
			$orderby_set = false;
			$page_id = themerex_get_template_page_id('learndash');
		} else if ($query->get('post_type')=='courses' || $query->get('courses_group')!='' || $query->get('courses_tag')!='') {
			$page_id = themerex_get_template_page_id('courses');
		...
		} else if ($query->is_posts_page==1 || themerex_strpos($THEMEREX_GLOBALS['page_template'], 'blog-')!==false) {
...
// AJAX: Login user
// Handler of add_action('wp_ajax_login_user',			'themerex_callback_login_user');
// Handler of add_action('wp_ajax_nopriv_login_user',	'themerex_callback_login_user');
if ( !function_exists( 'themerex_callback_login_user' ) ) {
	function themerex_callback_login_user() {

		if (!themerex_get_theme_option('ajax_login')) return;
		...
		$user_pwd = themerex_substr($_REQUEST['user_pwd'], 0, 30);
		...
		wp_set_current_user($rez->ID);
		wp_set_auth_cookie($rez->ID);
    ...
    function themerex_get_privacy_text() {
        ...
        return apply_filters( 'themerex_filter_privacy_text', wp_kses_post(
                ...
                . ( ! empty( $page ) && ! empty( $privacy_text )
                    ...
                    ? ' ' . sprintf( __( 'For further details on handling user data, see our %s', 'education' ),
                        '<a href="' . esc_url( get_permalink( $page ) ) . '" target="_blank">'
                        . __( 'Privacy Policy', 'education' )
                        . '</a>' )
                    : ''
                )
            )
