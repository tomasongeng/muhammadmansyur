...
if ( !function_exists( 'themerex_template_accordion_theme_setup' ) ) {
	...
	function themerex_template_accordion_theme_setup() {
		themerex_add_template(array(
			...
			'mode'   => 'blogger',
			'title'  => __('Blogger layout: Accordion (Style 1)', 'education')
			));
		themerex_add_template(array(
			'layout' => 'accordion-2',
			'template' => 'accordion',
			...
			'mode'   => 'blogger',
			'title'  => __('Blogger layout: Accordion (Style 2)', 'education')
			));
		...
		<div class="post_item sc_blogger_item sc_accordion_item<?php echo ($post_options['number'] == $post_options['posts_on_page'] && !themerex_sc_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : ''); ?>">
			...
			<div class="post_content sc_accordion_content">
				...
				if (themerex_sc_param_is_on($post_options['info'])) {
					?>
					<div class="post_info">
						<span class="post_info_item post_info_posted_by"><?php _e('Posted by', 'education'); ?> <a href="<?php echo esc_url($post_data['post_author_url']); ?>" class="post_info_author"><?php echo esc_html($post_data['post_author']); ?></a></span>
						<span class="post_info_item post_info_counters">
							<?php echo ($post_options['orderby']=='comments' || $post_options['counters']=='comments' ? __('Comments', 'education') : __('Views', 'education')); ?>
